...
#'
#' @export
optimize_weights_fromDF <- function(input_file, bias_file = NA,
  ...
  # The following was copied from read-data.R
  # But only copied the relevant bits
  ...
  long_names <- full_names
  short_names <- abbr_names
  data <- in_data
  n <- n
  ...
  # TODO: This needs to be fixed for cases where the input is already a DF so no name is required
  if (is.na(model_name)) {
