test_that("Basic cross-validation calls", {
  ...
  cv_results <- cross_validate(data_file, 10, 0, 1, in_sep=',')
  expect_equal(nrow(cv_results), 1)

  # Test call with vectors of scalars
  cv_results <- cross_validate(data_file, 10, c(0, 1, 2), c(0.01, 0.1, 1), in_sep=',')
  expect_equal(nrow(cv_results), 3)

  # Test call with grid search
  cv_results <- cross_validate(
    data_file, 10, c(0, 1, 2), c(0.01, 0.1, 1), grid_search=TRUE, in_sep=','
  )
  expect_equal(nrow(cv_results), 9)

  # Test call with list of vectors
  cv_results <- cross_validate(
    data_file, 10, list(c(0, 1, 2, 3), c(3, 4, 5, 6)),
    list(c(0.01, 0.1, 1, 2), c(2, 1, 0.1, 0.01)), in_sep=','
  )
