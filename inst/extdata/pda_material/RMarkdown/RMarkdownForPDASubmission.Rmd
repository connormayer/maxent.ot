...
model_MaxStressed_SSP_bias1 <- optimize_weights(MaxStressed_SSP_input, mu_scalar = 0, sigma_scalar = 1)
...
```{r elaborate_bias}
model_MaxStressed_SSP_bias2 <- optimize_weights(MaxStressed_SSP_input, mu_vector = c(2,0,0,1), sigma_vector = c(1, 1, 0.1, 1))
...
```{r close_fit}
shona_1000 <- optimize_weights(shona_input, mu_scalar = 0, sigma_scalar = 1000, upper_bound = 10)
...
shona_1000_crossval_model <- optimize_weights(training_tableau, mu_scalar = 0, sigma_scalar = 1000, control_params = NA, upper_bound = 10) 
...
```{r sigma_0_point_1}
#Fit model to training data
shona_point01_crossval_model <- optimize_weights(training_tableau, mu_scalar = 0, sigma_scalar = 0.1, upper_bound = 10)
...
```{r crossval_sigma_3}
#Fit model to training data
shona_3_crossval_model <- optimize_weights(training_tableau, mu_scalar = 0, sigma_scalar = 3, upper_bound = 10)
