...
optimize_weights(
  data_frame, mu_vector = c(1, 2), sigma_vector = c(100, 200)
)
```
...
optimize_weights(
  data_frame, mu_scalar = 0, sigma_scalar = 1000
)
```
...
optimize_weights(
  data_frame, mu_vector = c(1, 2), sigma_scalar = 1000
)
```
...
regularized_model <- optimize_weights(
  data_frame, mu_scalar=0, sigma_scalar=1
)
```
...
white_salt_bias_model = optimize_weights(
  white_salt_data_frame, mu_vector=c(0, 0, 1.3, 3.65), sigma_scalar=sqrt(0.6)
)
