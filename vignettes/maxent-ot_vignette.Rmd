...
data_table <- read.table(data_file, header=FALSE, sep='\t')
data_table
```
...
# Take a look at the structure of the bias_df
bias_df
```
...
optimize_weights(data_df, bias_df)
```
...
optimize_weights(
  data_df, mu_vector = c(1, 2), sigma_vector = c(100, 200)
)
```
...
optimize_weights(
  data_df, mu_scalar = 0, sigma_scalar = 1000
)
```
...
optimize_weights(
  data_df, mu_vector = c(1, 2), sigma_scalar = 1000
)
```
...
regularized_model <- optimize_weights(
  data_df, mu_scalar=0, sigma_scalar=1
)
```
...
As with `optimize_weights`, you can also pass a data frame/data table/tibble into `predict_probabilities` instead of a file path.

``` {r}
# Get paths to toy data file
data_frame_file_a <- system.file(
  "extdata", "sample_data_frame.csv", package="maxent.ot"
)
data_frame_a <- read.csv(data_frame_file_a)

# Fit weights to data (no biases)
fit_model_a <- optimize_weights(data_frame_a)

# Predict probabilities for the same input
predict_probabilities(data_frame_a, fit_model_a$weights)
```
