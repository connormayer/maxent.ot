...
Higher values of $LL_w(D)$ and lower values of $B(w)$ both contribute to increasing the value of the objective function $J(w)$.
...
(1) improve the model's match to the observed distribution: higher $LL_w(D)$
(2) move weights closer to the preferred weight values: lower $B(w)$
...
When no priors are used, maximizing the objective function $J(w)$ is equivalent to maximizing the log likelihood of training data $LL_w(D)$ alone.
...
In the previous section (&sect;2), we ignored the second term, so objective function, $J(w)$, depended solely upon the log likelihood of the training data under the current weights.
...
This price is subtracted from the log likelihood ($LL_w(D)$), and thus makes the objective function, $J(w)$ smaller (notice the negative sign preceding the second term).
...
t1_pred$predictions[, 'Predicted Probability']
```

Now we'll create the data frame: 
```{r}
# Select columns we want, and store them in lettered variables
a <- t1_pred$predictions[, 'UR']
b <- t1_pred$predictions[, 'SR']
c <- t1_pred$predictions[, 'Predicted Probability']
d <- t3_pred$predictions[, 'Predicted Probability']
e <- t5_pred$predictions[, 'Predicted Probability']
...
#hu_simul_wts <- monte_carlo_weights(hu_pred, 2)
#hu_simul_wts
```
